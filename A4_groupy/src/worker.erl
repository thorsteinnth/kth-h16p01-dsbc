...
% This is the application layer process

% Start a new application layer process (worker)
...
join(Id, Cast) ->
    receive 
			{view, _} ->
					Ref = make_ref(),	% Make unique reference among connected nodes
...
% Seed randomness, start GUI, enter worker loop
init_cont(Id, Rnd, Cast, Color, Sleep) ->
...
% The worker process, 
% Cast is the PID of the group process
worker(Id, Cast, Color, Gui, Sleep) ->
    Wait = wait(Sleep),
    receive
